...
| **GitHub backup repo created (`odarambmenezes/odyssey`), kept private** (corrected 2026-09-01) | Notion publicly acknowledged an MCP tool-call reliability issue (2026-08-31) that stalled work mid-session. Repo holds a manual markdown mirror of docs (Codex, Playbook, GM Charter, Governance, Skills) as a fallback. No personal task data ever committed here — that stays in private Notion only. An earlier log entry said "made public" but that never actually took effect; once discovered, Odara chose to keep it private. Privacy doesn't block the sync workflow — Claude Code has its own authenticated access regardless of visibility; the only reason to go public would be letting the claude.ai chat (which has no token) read it directly, which isn't essential. | Locked |
